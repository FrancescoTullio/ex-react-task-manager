...
const AddTask = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (newObj.title.trim().length > 0 && newObj.description.trim().length > 0 && newObj.status !== "") {
      addTask(newObj)
      setForm(defaultForm)
  ...
  return (
    <>
      ...
      <div className="container">
        ...
        <h1 className="text-center mb-5">Aggiungi una task a quelle precedenti</h1>
        <form onSubmit={handleSubmit}>
          <div className="mb-3">
            ...
            />
            {
              (form.title.trim().split('').some(char => symbols.includes(char)) || form.title.trim().length === 0) &&
              <p className="m-2 text-danger">
                Il titolo del task non puo contenere caratteri speciali e non puo essere vuoto
              </p>
            }
          </div>
          <div className="mb-3">
            <label className="form-control text-center">Destrizione</label>
            <textarea name="description" ref={areaDes} className="w-100"></textarea>
          </div>
          <div className="mb-3">
